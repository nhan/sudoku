...
(function() {
  ...
  Sudoku.arrayEquals = function(a, b) {
    return !(a < b || b < a);
  };

  Sudoku.log = function(message) {
  ...
  Sudoku.Cell = Ember.Object.extend({
    ...
    toString: function() {
      return "cell[" + (this.get('x')) + ", " + (this.get('y')) + "]";
    }
  });
  ...
  Sudoku.Board = Ember.Object.extend({
    init: function() {
      ...
      for (x = _i = 0; _i < 9; x = ++_i) {
        for (y = _j = 0; _j < 9; y = ++_j) {
          Sudoku.CannotHaveSameValueConstraint.create({
            ...
            relatedCells: this.blockAt(x, y).addObjects(this.rowAt(x, y)).addObjects(this.columnAt(x, y)).without(this.cellAt(x, y))
          });
        }
      }
      ...
      return this.get('blocks').forEach(function(block) {
        return Sudoku.AllValuesMustAppearConstraint.create({
          cells: block
        });
      });
    ...
    blockAt: function(x, y) {
      ...
      for (i = _i = 0; _i < 3; i = ++_i) {
        for (j = _j = 0; _j < 3; j = ++_j) {
          ret.push(this.cellAt((x - (x % 3)) + i, (y - (y % 3)) + j));
        }
      }
      return ret;
    },
